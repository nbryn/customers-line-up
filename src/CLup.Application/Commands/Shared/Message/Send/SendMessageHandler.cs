    ...
    public class SendMessageHandler : IRequestHandler<SendMessageCommand, Result>
    {
        ...
        public async Task<Result> Handle(SendMessageCommand command, CancellationToken cancellationToken)
        {
            return await _context.Users.FirstOrDefaultAsync(user => user.Id == command.SenderId)
                    ...
                    .AndThen(async user =>
                    {
                        var business = await _context.Businesses.FirstOrDefaultAsync(business => business.Id == command.SenderId);
                    ...
                    .AndThen(async () => 
                    {
                        ...
                        var business = await _context.Businesses.FirstOrDefaultAsync(business => business.Id == command.ReceiverId);
                        return new { business, user };
                    })
