    ...
    public class DeleteBookingHandler : IRequestHandler<BusinessDeleteBookingCommand, Result>
    {
        ...
        public async Task<Result> Handle(BusinessDeleteBookingCommand command, CancellationToken cancellationToken)
            => await _context.Businesses.Include(business => business.Owner)
                .FirstOrDefaultAsync(x => x.Id == command.BusinessId)
                .ToResult()
                .EnsureDiscard(business => business.Owner.Email == command.OwnerEmail,
                    (HttpCode.Forbidden, "You don't have access to this business"))
                .FailureIf(
                    () => _context.Bookings
                        .Include(booking => booking.Business)
